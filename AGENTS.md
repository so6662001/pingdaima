...
目前仓库里只有两样东西：**24 个 HTML 高保真原型**（根目录）与**完整 PRD 文档**（`docs/`）。应用代码一行还没写，`apps/`、`packages/`、`prisma/` 都还不存在——第一个任务 T-0001 就是把它们建起来。
...
2. **环境自检**：Node ≥ 20、pnpm、Docker（Postgres/Redis）。缺什么装什么，不要问用户。
3. **建立待办清单**：用 todo 工具把本批次任务逐条登记。
4. **逐任务执行**，每个任务走完整闭环：
   实现 → 自检（lint + typecheck + test + 相关校验脚本）→ 更新 `docs/dev/TASKS.md` 打勾并记录 commit → **单独提交一个 commit**（禁止多任务合并提交）
...
3. **样式原样迁移**。`assets/css/app.css` 整体复制到 `packages/ui/styles/`，禁止重写、禁止改类名、禁止改用 Tailwind/CSS-in-JS 重新实现。
4. **图表与图标原样移植**。`assets/js/charts.js`、`assets/js/icons.js` 移植为 TS 模块/组件，禁止替换为 ECharts、Ant Design Icons 等第三方库。
5. **算法参数集中管理**。90 分钟聚类窗口、180 行基准当量、0.78 相似度阈值这类数值，全部集中在 `packages/shared/src/domain/` 下的常量文件，每个常量必须带 `@prd` 注解指向 PRD 出处，禁止散落在业务代码里。
6. **状态流转必须走状态机**。任何状态变更都必须经过统一的状态机服务校验守卫条件，禁止在业务代码里直接 `update({ status })`。
...
```
apps/
  web/                 前端 React + TS + Vite（页面与原型 1:1）
  api/                 后端 NestJS + TS + Prisma
packages/
  shared/              前后端共享：类型、枚举、状态机定义、算法、指标口径、种子数据
  ui/                  设计系统：原型 CSS 原样迁入 + 组件 + 图表引擎 + 图标
prisma/                数据库 schema 与迁移（对应 docs/prd/data-model.md）
e2e/                   Playwright 端到端与视觉回归
docs/                  PRD 与开发文档（已存在，不要删改结构）
tools/                 校验脚本（已存在）
*.html                 原型页面（**只读参考，禁止修改、禁止删除**）
```
...
pnpm e2e:parity      # 视觉回归（应用 vs 原型）
pnpm check:prd       # 校验代码中的 @prd 注解指向的章节真实存在
pnpm db:seed         # 灌入与原型完全一致的种子数据
```
